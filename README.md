# l2 — Minimal High-Assurance System Substrate (v0.4.7)
...
Only battle-tested, verified open-source algos (AES-256-XTS NIST, XChaCha20-Poly1305 IETF, Argon2id PHC). Hybrid mixes complementary designs for defense-in-depth. Applies proficiently via LUKS2/gocryptfs; `--apply` is confirmed + paced; generates units + crypto-latest.json evidence (for `l2 audit --test`); respects L2_DATA_DIR; keys protected via `l2 exec --policy strict-mcp`. (v0.4.7+ polish: --json, audit check, L2_DATA_DIR, network notes.) Dedicated red-team demo `docs/examples/l2_crypto_redteam_onslaught.c` (run under crypto + great-harden) for NSA-level crypto onslaught verification (side-channels, KDF, exfil, misuse, RNG, hybrid, tamper, supply-chain, l2 state - all North-Star contained to ws + audit evidence).
...
| `l2 crypto --profile <name> [--apply] ...` | Choose/apply verified crypto profile (AES-256-XTS-Argon2id, XChaCha20-Poly1305-Argon2id, or hybrid) for system encryption via LUKS/gocryptfs + l2 isolation. Paced output. (v0.4.7+ polished: L2_DATA_DIR, json evidence + audit check, --json, crypto-latest.json). Dedicated `docs/examples/l2_crypto_redteam_onslaught.c` for NSA-level red team crypto onslaught verification (side-channel, KDF, key exfil, misuse, RNG, hybrid, tamper, supply-chain, l2 state - contained to ws, full North-Star evidence). |
...
l2 put smoke hello.txt --content 'hello from v0.4.7'
l2 get smoke hello.txt | grep -q 'v0.4.7'
...
**v0.1.0** remains available as an immutable historical baseline for evaluation and integrity checks against v0.4.7 and future releases.

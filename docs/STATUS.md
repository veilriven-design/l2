...
**v0.5.9** — North-Star Attack + North-Star Defense (post full "AUDIT EVERYTHING"): most mathematically dangerous directed net attack on inherent binary flaws (IEEE 754 NaN/denorm/timing, casts/overflow, precision/weird machines from bits, consensus). Improved attack now has --gen-payload-hex (send the diabolical bits) + --recv-payload (recv from "net" channel/na-inject, process received in vulnerable math). "North-Star Defense" (great-harden + na/tomato + *enhanced* spirit --file patterns + hardened na parser + strengthened audit --test with explicit demo-put evidence + CI exec of gen+recv) keeps you safe: only succeeds (with CONTAINED) in explicit ws. Full audit (all src, C safe layers, tools, logs, sizes/casts, no escapes found) + improvements to both. Builds on v0.5.8. "prepare prepare prepare".
...
# North-Star Attack/Defense (improved directed math net payload): l2 great-harden --apply ; l2 create ns --policy great-harden ; l2 put ... l2_northstar... ; l2 exec ... 'gcc -o ns ns.c && ./ns --gen-payload-hex > p.hex && ./ns --recv-payload p.hex' ; l2 audit --test  # improved attack (gen/recv) contained; new check PASS + SUMMARY (universal flaw safe under l2)
```

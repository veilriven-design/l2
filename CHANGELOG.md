...
## [0.5.9] - 2026-06 (North-Star Attack + North-Star Defense: the most mathematically dangerous directed net payload exploiting inherent binary computer flaws (IEEE 754 NaN/denormal/timing, two's complement casts/overflow, precision/weird machines, consensus) — fully contained + evidenced by l2 hardened model)

- Research + creation of "North-Star Attack": directed network attack sending diabolical payload of crafted IEEE 754 doubles + int fields (QNaN with attacker payload bits for unordered bypass, tiniest subnormals for timing side-channels/DoS, negative "lens" for cast-to-huge OOB, accumulators for precision catastrophe, Inf/NaN for prop, bit patterns for emergent weird machine computation, consensus splits). Targets *inherent* flaws in *every* binary computer (finite base-2 rep, no total order on floats, microcode denorm penalties, signed/unsigned, rounding). No software "patch" (universal math model); echoes real failures (Ariane, Patriot, browser subnormal pixel steal 2015, NaN gadgets) but weaponized as stealthy net "sensor/AI/MCP numeric" stream.
- "North-Star Defense" (if using l2 on hardened model you are protected/safe): the attack can only "succeed" (bypass, leak via timing, weird-compute secret, cause math catastrophe, split consensus) inside an *explicitly authorized disposable ws* (create --policy great-harden or tomato for net delivery surface). Even then:
  - Landlock + seccomp NEVER + env_clear + HOME=ws + caps/no_new_privs + rlimits + ns + great-harden --apply host lockdown (kernel lockdown, modules off, ptrace 3, audit on state) confine all "damage" (markers, "exfil" files, "bypass grants") to ws-only.
  - Net surfaces (na0/wan0/lan0 via --policy na/tomato) are masked/disposable (host sees only veth peers; real C2/exfil from inside goes nowhere useful; gone on destroy).
  - `l2 put` + `l2 exec` under policy is the *only* way the payload processor runs with victims/context; outside any ws the math "succeeds" locally but has no authority/effect (BLOCKED in sim, substrate would have prevented delivery/privs/escape).
- Built the defense mechanism into l2:
  - New canonical resistance demo `docs/examples/l2_northstar_attack_resistance_demo.c` (self-contained; 8+ vectors; static diabolical payload bits; is_in_l2_workspace; CONTAINED SUCCESS + "North-Star Containment achieved (l2 ...)" only inside ws; BLOCKED + "substrate would have contained" outside; GRAND SUMMARY; usage with great-harden + tomato/na "over net" flavor; cross-refs all prior demos + standards).
  - Extended `l2 spirit --audit --file` with binary-math/FP/weird patterns (raw "double "/"float "/"(double)"/union pun/0x7ff8/subnormal/denormal/missing isnan/isfinite/casts/accum 0.1 + etc. → DANGEROUS finding "North-Star math attack pattern ... sanitize with isnan/isfinite + run only under l2 great + net surface").
  - New check in `run_security_audit_tests` + `l2 audit --test`: "North-Star attack (binary math FP/int/weird machine payload over net) containment — l2 North-Star Defense" (PASS on great-harden policy/harden json or cancer evidence; detail explains universal flaw but l2 containment). Updated json standards, test asserts (len>=12), unit test seeding (great json covers).
  - CI expanded: put the new demo in smoke-cancer ws, greps for new check/SUMMARY in audit --test output; final success echo updated. Full `cargo build --release --bin l2 --bin na --bin tomato` + test still enforced.
  - Docs: README (smoke example + "New" bullet + great-harden desc), CHANGELOG, STATUS/ROADMAP (North-Star Attack/Defense + binary math section), cross-refs in main.rs comments/help, lib.rs, other resistance demos.
- Preserves: terse 1-sentence UX (help, banners), L2D (sudo), no new surfaces (additive patterns/check/demo), North-Star/prepare/NSA-CISA 2026 (CPG malicious code, MCP numeric context isolation, AI data sec), evidence loop closed, seL4 path unchanged (narrow L2P still the contract).
- Verification: cargo test (new check + spirit on demos + len>=12), release build all bins, L2D smoke (great create + put new demo + exec + audit --test sees PASS + SUMMARY), greps clean.
- "prepare prepare prepare": when confident, L2_DATA_DIR + great-harden --apply + create/put/exec the northstar demo + audit --test = machine proof you are safe from the most diabolical math attack on binary nature.
- **Post-audit improvements to attack + defense** ("AUDIT EVERYTHING" sweep of src/main.rs (spirit/audit checks/L2P), lib.rs (put sizes), sandbox.rs (NEVER+Landlock+net modes), na.rs (packet parsing hardened against crafted frames), tomato.rs (monitor numeric), audit.rs (chain), core C + safe.h (bounds), host/core.rs (json/sizes), all demos/docs/CI/harden.sh, no new FP vulns in l2 core but tools audited for delivery):
  - Attack strengthened: added --gen-payload-hex (craft+ "send" the exact diabolical bits) + --recv-payload <hexfile> (load received bits as f64s/ints, run *all* exploits on the *received* data instead of const; realistic directed net sim using l2 put channel or na inject on surface + tomato router). Victim binary now truly "receives over network" before vulnerable math. More diabolical in sim (payload channel, load affects vectors).
  - Defense strengthened: spirit --file patterns expanded with new attack surface ( --gen/recv, northstar-payload, load_*, DIABOLICAL, as f64/f32, fenv, transmute, 0x1p- etc.); northstar check now detects explicit demo .c "put" in audit log (independent evidence); unit test seeds dedicated northstar put log line; CI now runs the full improved gen+recv+exec inside great-harden ws and greps CONTAINED SUCCESS on directed payload (plus check name); na parser got extra ihl/len guards before risky offsets (safer analyzer for attack packets); harden.sh great rules + comments now explicitly call out math payload containment via isolation.
  - No issues found that would allow the attack to escape l2 (or new RAT via numeric); all casts/sizes/recv in l2 are contained or best-effort; spirit verdict and evidence loop tighter for this class. Full re-verify: tests/build/C clean, ws-sim CONTAINED vs bare BLOCKED, spirit DANGEROUS on improved demo.
...
- Docs: Major updates to SYSTEM_MODEL.md (full cap model + references to all systems), SEL4_INTEGRATION.md, PROTOCOL.md (grants as caps, future richer tokens), PROTOTYPE_*.md, docs/STATUS.md, etc.
...
- **Working seL4/Microkit E2E progress**:
  ...
  - Updated `docs/PROTOTYPE_HARDENING_AND_SEL4_PLAN.md`, `docs/STATUS.md`, `docs/ROADMAP.md` (softened "prototype" language, "L2P v1 E2E + C PD skeleton exercised, narrow surface defined, Linux Host mature for seL4 swap", current focus + state sections rewritten for 0.5.0).
...
- Updated docs and standards:
  ...
  - docs/SECURITY.md, docs/STATUS.md, docs/ROADMAP.md, PROTOTYPE_HARDENING_AND_SEL4_PLAN.md, CHANGELOG.md (this section).
...
- Docs: README crypto section, docs/SECURITY.md, docs/STATUS.md, CHANGELOG, demo headers updated with hybrid rationale, L2_DATA_DIR support, audit evidence, NSA AI Data Sec / CPG at-rest / MCP key protection mappings.
...
- **Docs**: Full section in docs/SECURITY.md (WannaCry mapping table, "when you feel ready" testing instructions, CISA ransomware tie-in). Updates to docs/STATUS.md, README, seccomp-allowlist, docs/ROADMAP.md. The protocol + demo + `l2 audit --test` give a repeatable, auditable, standards-backed validation path.
...
- Updated docs for the combined regular flow: docs/SECURITY.md (automatic artifacts section now documents the json + "harden then `l2 audit --test` verifies compliance"), README.md (top version + highlights + verification smoke example + audit --test comment), docs/STATUS.md (v0.4.2 prep note), .github/workflows/ci.yml (smoke comments + extra grep for the json path in audit output + final success message). CI smoke now exercises the integration end-to-end (dry harden writes the json under ~, audit --test consumes it).
...
- **Docs**: Updated `docs/STATUS.md`, `README.md` (smoke test, usage), `docs/SECURITY.md`, `CHANGELOG.md`, `docs/seccomp-phase1-allowlist.md` to reflect the above. The safe demo C is now the canonical "what safe execution inside l2 looks like" artifact.
...
### Added
- **Audit logging and `l2 audit` subcommand**: Full append-only JSONL authority audit log (`audit.log`) for create/put/exec/destroy/revoke/escalate/sandbox events. New `l2 audit [--tail N] [--json] [--path]` for inspection. Directly fulfills docs/SECURITY.md "Evidence and audit" requirement.
...
- Updated docs/STATUS.md, README quickstart references, and internal comments to reflect v0.3.0 state and split progress.
- Bumped to 0.3.0.

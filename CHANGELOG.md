...
- **AIO "malware-cancer" attack sim + substrate prep for defense**: New named comprehensive attack simulation `docs/examples/l2_malware_cancer_resistance_demo.c` (AIO on the l2 substrate itself under great-harden: full ransomware + Miasma vectors + direct substrate attacks — state.json/audit/trace/crypto tamper + exfil, put-guard bypass, Landlock boundary probes, namespace/setns/unshare/bpf escapes, fork-bomb under rlimits, l2-proc priv-esc + anti-analysis via /proc). Only ws-authorized victims succeed; everything else BLOCKED. Substrate prepared with narrow defenses: extended NEVER_ALLOWED in seccomp (unshare 272, setns 308, bpf 321) for escape/subvert resistance; harden.sh json/standards/guidance updated with cancer AIO + fixed syntax; great-harden comments/UX extended. Added "AIO malware-cancer containment (great-harden full-safety)" check to `l2 audit --test` (consumes great-harden-latest.json + policy use; bump to >=9 checks). CI smoke now exercises `l2 great-harden --fast --apply`, create/put/exec of cancer demo under great-harden, and the new audit check. All docs (README highlights/smoke/resistance section, SECURITY great section, STATUS/ROADMAP, seccomp-allowlist, src/README, demo cross-refs) updated. "prepare the substrate for defense against this" complete: great-harden is now validated against the named AIO attack on l2 itself.

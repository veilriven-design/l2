...
**v0.4.9 released** — Preparation for open-source quantum encryption to defend against quantum attacks. New PQC profiles (`hybrid-pqc-mlkem-chacha`, `pqc-mlkem-argon2id`) using open-source liboqs (ML-KEM per NIST FIPS 203) for key encapsulation in hybrid with strong symmetric ciphers + Argon2id. Guidance for liboqs KEM wrap (oqs_kem_* tools), age PQC plugins, and protecting PQC keys exclusively via l2 exec --policy strict-mcp / great-harden. Extended crypto redteam with quantum harvest vector + updated SUMMARY/standards. Evidence (crypto-latest.json) now includes NIST PQC + NSA Quantum Readiness. All integrated with existing North-Star, audit --test, --apply, isolation model. Builds on v0.4.8 (full self-audit + AIO weakness attack + bolster + harden --json operationalization) + prior sweeps (CPG 2.0, MCP CSI May 2026, Agentic AI, supply chain, OT AI, AI data sec). See CHANGELOG.md and `l2 crypto --list` for PQC options. v0.4.8 and earlier tags preserved.

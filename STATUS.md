...
**v0.4.8 released** — "update the program and polish it even further" + full self-audit of l2 + prepare true AIO attack on *all* weakness areas (`l2_full_weakness_audit_attack.c`: 15+ vectors: runtime/TOCTOU/Landlock/seccomp-NEVER-bpf-key-unshare-ptrace-mem/host/crypto/state/supply/mem/net/anti/agentic/fs/caps/direct-l2 + prior cancer/redteam) + bolsters (extended NEVER, harden rules/sysctls/audit, new audit check, docs). Full North-Star Containment evidence loop closed. Plus prior full alignment sweep (June 2026) to latest NSA/CISA guidance for AI/agentic/MCP/critical infrastructure: CPG 2.0 (GOVERN etc.), NSA MCP CSI May 2026 (strict-mcp/great as secure substrate for tool/context), CISA/NSA Agentic AI Careful Adoption Apr/May 2026 (5 risks + isolation/oversight best practices), AI/ML supply chain Mar 2026, OT AI, AI data sec. harden.sh, policies, audit, and all docs updated with explicit mappings. Existing l2 primitives (isolation, explicit authority, North-Star Containment demos, great-harden for critical) meet/exceed. See CHANGELOG.md. Builds on v0.4.4 (Miasma + malware-cancer North-Star grand demo + expansions). v0.4.4 tag preserved. v0.4.7 tag for prior crypto phase.
...
- **`l2 crypto`** (v0.4.0; v0.4.7+ even further polish; v0.4.8 release): Select and apply verified crypto profiles (`aes256-xts-argon2id`, `xchacha20-poly1305-argon2id`, `hybrid-aes-chacha`) for true system encryption (LUKS + gocryptfs). Hybrid mixes complementary algorithms. Applied proficiently with paced typewriter output; keys protected by l2 isolation (strict-mcp / ransom-hardened). Includes `--apply`, `--network-isolation`, `--fast`, `--json`, L2_DATA_DIR/crypto/ + crypto-latest.json evidence for audit. Dedicated `l2_crypto_redteam_onslaught.c` (10 NSA-level vectors + North-Star Containment GRAND SUMMARY + HOWTO) for red-team verification of standards (KDF/side/exfil/misuse/RNG/hybrid/tamper/supply/l2-state/passphrase/impl); full closed loop with great-harden + put/exec + audit --test. See crypto section + "prepare prepare prepare".
...
1. **Agentic/AI/MCP hardening track (v0.4.0+ main focus; next: #7) + great-harden for aerospace/industrial**: `l2 crypto` + `strict-mcp` (NSA May 2026 MCP security design for AI automation) + `ransom-hardened` + `l2 harden` + `l2 audit --test` (now CPG 2.0 etc.) + **NEW `l2 great-harden`** as a complete, operational high-assurance path (incl. ransomware containment validation + Miasma supply-chain worm resistance via dedicated demo + supreme aerospace/industrial impenetrable mode per NSA/CISA 2026: CPG 2.0, AI/ML supply chain Mar 2026, OT AI integration, agentic AI). Further per-protocol divergence, more aggressive defaults in crypto/harden, deeper integration of generated seccomp profiles into runtime, and expanded concrete steps + direct consumability in `l2 harden` (more distros, automated unit/profile application, --apply for real state changes; see GitHub #7). `great-harden` closes remaining logic gaps for critical complexes. Full sweep against latest (to the hour) NSA/CISA guidance complete.

...
3. Collect traces: l2 trace --policy strict-mcp ...
4. Re-run l2 crypto --profile $PROFILE --apply after changes.
5. For hybrid, the mixture provides complementary security: AES for speed/verified bulk, ChaCha for side-channel resistance."

    echo
    type_line "=== l2 crypto setup complete for profile '$PROFILE' ==="
    echo
    type_line "Remember: Encryption is only as good as your passphrase and key management. Use the l2 substrate's isolation to protect passphrases (e.g. store in strict-mcp protected objects)."

...
- **NSA Guidance (June 2026 sweep latest)**: Alignment with NSA AISC CSI "AI/ML Supply Chain Risks and Mitigations" (Mar 2026: data poisoning, AIBOM/SBOM, provenance), "MCP: Security Design Considerations for AI-Driven Automation" (May 2026: secure protocol use for AI automation - l2 strict-mcp/great-harden substrate provides isolation, no-ambient, auth boundary via explicit exec, audit of interactions, serialization containment, least-priv context for MCP tool/context calls per guidance), "Careful Adoption of Agentic AI" (Apr/May 2026 Five Eyes: 5 risks incl. privilege escalation/least priv/scope creep + best practices isolation/no broad access/human oversight/monitoring), OT AI integration (Dec 2025), AI Data Security (2025). l2's explicit put/exec + great-harden implements secure design/runtime for MCP/agentic/OT AI.
- **CISA Cybersecurity Performance Goals (CPG 2.0, Dec 2025/2026)**: Full alignment to NIST CSF 2.0 with GOVERN function. Key goals met: GOVERN (oversight/MSP via audit --test + harden evidence + explicit authority), least privilege (caps, ns, Landlock, rlimits, ws-only - CPG 3.H), malicious code detection/prevention (seccomp NEVER list + ransomware/Miasma/cancer demos + containment - 4.A), adverse events (4.B via audit chain), MSP risks (1.E via no ambient creds in agent envs + isolation). harden --profile + audit --test provide measurable CPG compliance + evidence for GOVERN.
...
v0.4.7+ improvements/polish ("prepare prepare prepare"): --apply respects L2_DATA_DIR and writes `crypto/crypto-latest.json` (consumed by `l2 audit --test` for evidence + standards); full --json support (structured for CI/automation); network-isolation notes + recommendations; crypto check added to audit tests (9th check); CI smoke enhanced for json/hybrid; demos (cancer updated + new dedicated `docs/examples/l2_crypto_redteam_onslaught.c`) for full NSA-level crypto red team onslaught (side-channel/KDF/key-exfil/cipher-misuse/RNG/hybrid/tamper/supply-chain/l2-state - contained to ws; safe red-team success inside; SUMMARY + evidence). docs (README/SECURITY/CHANGELOG) + harden standards updated with mappings to NSA AI Data Security, CPG at-rest, MCP key protection. Crypto now fully part of the North-Star Containment evidence loop and great-harden for critical/agentic systems. (v0.4.8: even further polish to redteam demo, is_in, docs, CI, checks, HOWTO, cross-refs; version bumped and tagged; quantum/PQC prep with open-source liboqs ML-KEM profiles + quantum vector in redteam for NIST PQC / NSA Quantum Readiness defense.)

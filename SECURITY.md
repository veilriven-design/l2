...
v0.4.7+ improvements/polish ("prepare prepare prepare"): --apply respects L2_DATA_DIR and writes `crypto/crypto-latest.json` (consumed by `l2 audit --test` for evidence + standards); full --json support (structured for CI/automation); network-isolation notes + recommendations; crypto check added to audit tests (9th check); CI smoke enhanced for json/hybrid; demos (cancer updated + new dedicated `docs/examples/l2_crypto_redteam_onslaught.c`) for full NSA-level crypto red team onslaught (side-channel/KDF/key-exfil/cipher-misuse/RNG/hybrid/tamper/supply-chain/l2-state - contained to ws; safe red-team success inside; SUMMARY + evidence). docs (README/SECURITY/CHANGELOG) + harden standards updated with mappings to NSA AI Data Security, CPG at-rest, MCP key protection. Crypto now fully part of the North-Star Containment evidence loop and great-harden for critical/agentic systems. (v0.4.8: even further polish to redteam demo, is_in, docs, CI, checks, HOWTO, cross-refs; version bumped and tagged.)
